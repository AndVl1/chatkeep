...
### 4. Log Verification
```
get_logs(package: "com.chatkeep.admin", level: "E")
...
#### launch_app
```
launch_app(package: "com.chatkeep.admin")      # Android
launch_app(package: "com.chatkeep.admin")      # iOS (bundle ID)
```

#### stop_app
```
stop_app(package: "com.chatkeep.admin")        # Force stop
```
...
get_logs(package: "com.chatkeep.admin")         # Filter by app
...
shell(command: "privacy grant all com.chatkeep.admin")
```
...
#### open_url
```
open_url(url: "https://chatkeep.ru")            # Open in browser
```
...
ANR in com.chatkeep.admin           -> App not responding
java.lang.NullPointerException      -> Null reference crash
java.lang.OutOfMemoryError          -> Memory issue
Process: com.chatkeep.admin, PID... -> Crash with stack trace
```
...
### Scenario: App Launch
1. `clear_logs()`
2. `launch_app(package: "com.chatkeep.admin")`
3. `wait(ms: 3000)`
...
8. `wait(ms: 2000)`
9. `launch_app(package: "com.chatkeep.admin")` -> Resume
...
### Scenario: Deep Link
1. `stop_app(package: "com.chatkeep.admin")`
2. `shell(command: "am start -a android.intent.action.VIEW -d 'chatkeep://chat/123'")`
3. `wait(ms: 2000)`
...
adb shell dumpsys meminfo com.chatkeep.admin    # Memory usage
...
# App control
adb shell pm clear com.chatkeep.admin           # Clear app data
adb shell am force-stop com.chatkeep.admin      # Force stop
adb shell am start -a android.intent.action.VIEW -d "chatkeep://path"  # Deep link
...
xcrun simctl uninstall booted com.chatkeep.admin  # Uninstall
xcrun simctl launch booted com.chatkeep.admin   # Launch app
xcrun simctl terminate booted com.chatkeep.admin  # Terminate

# Permissions
xcrun simctl privacy booted grant all com.chatkeep.admin   # Grant all
xcrun simctl privacy booted reset all com.chatkeep.admin   # Reset
